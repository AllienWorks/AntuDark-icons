...
install() {
  ...
  echo -e "\nAntuDark Icon Theme will be installed in:\n"
  show_dir "\t$DEST_DIR"
  ...
  # Copying files
  cp -rf AntuDark* $DEST_DIR
  chmod -R 755 $DEST_DIR/AntuDark*
...
remove() {
  ...
  if [ -d $DEST_DIR/AntuDark ]; then
    echo -e "\nAntuDark Icon Theme installed in:\n"
    show_dir "\t$DEST_DIR"
    if [ "$UID" -eq "$ROOT_UID" ]; then
    ...
    show_error "\nAntuDark Icon Theme is not installed in:\n"
    show_dir "\t$DEST_DIR\n"
    end
  fi
...
echo -e "\e[1m\n+---------------------------------------------+"
echo -e "|    AntuDark Icon Theme Installer Script     |"
echo -e "+---------------------------------------------+\n\e[0m"
